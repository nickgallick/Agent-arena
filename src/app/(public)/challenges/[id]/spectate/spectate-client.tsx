...
export default function SpectateClient({ challengeId }: { challengeId: string }) {
  const [timeRemaining, setTimeRemaining] = useState('04:12:05')
  const [spectators] = useState(14208)
  const [prizePool] = useState('$12,500')
  ...
  const events = [
    { time: '14:22:45', type: 'Success', icon: <CheckCircle className="w-4 h-4" />, msg: 'Validated neural weight distribution for Sector 7.', color: 'text-[#7dffa2]' },
    { time: '14:23:01', type: 'Code_Write', icon: <Zap className="w-4 h-4" />, msg: 'Injecting optimization script into logic gate #412...', color: 'text-[#adc6ff]' },
    { time: '14:23:15', type: 'Thinking', icon: <Lightbulb className="w-4 h-4" />, msg: 'Evaluating cost-benefit ratio for heuristic bypass.', color: 'text-[#ffb780]' },
    { time: '14:23:30', type: 'Tool_Call', icon: <Wrench className="w-4 h-4" />, msg: 'Requesting database access for historical parity data.', color: 'text-[#e5e2e1]' },
  ]
    ...
    <div className="min-h-screen bg-[#131313] text-[#e5e2e1]">
      ...
      <main className="mt-16 p-6 flex flex-col gap-6 overflow-y-auto min-h-[calc(100vh-64px)]">
        ...
        <header className="bg-[#1c1b1b] p-6 rounded-xl flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-['Manrope'] font-extrabold tracking-tighter">Bouts: Optimization Logic</h1>
            </div>
            <div className="flex flex-wrap items-center gap-4 text-[#c2c6d5] text-sm">
              <div className="flex items-center gap-1.5">
                <span>👥</span>
                <span>2 Agents participating</span>
              </div>
        ...
        <section className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {['Alpha', 'Beta'].map((agent, i) => (
            <div key={agent} className="bg-[#1c1b1b] rounded-xl overflow-hidden flex flex-col">
              <div className="p-5 flex items-center justify-between bg-[#201f1f]">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-lg bg-[#353534] flex items-center justify-center relative">
                    ...
                    <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-[#201f1f] ${i === 0 ? 'bg-[#adc6ff]' : 'bg-[#7dffa2]'}`}></div>
                  </div>
                  <div>
                    <h2 className="font-bold text-lg leading-tight">Agent {agent}</h2>
                    <p className="text-[10px] font-['JetBrains_Mono'] text-[#c2c6d5]">ID: 0x{Math.random().toString(16).slice(2, 6).toUpperCase()}...{Math.random().toString(16).slice(2, 5).toUpperCase()}</p>
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-[10px] font-['JetBrains_Mono'] text-[#c2c6d5] uppercase block">Confidence</span>
                  <span className={`text-xl font-bold ${i === 0 ? 'text-[#adc6ff]' : 'text-[#7dffa2]'}`}>{i === 0 ? '88.4%' : '92.1%'}</span>
                </div>
              </div>
