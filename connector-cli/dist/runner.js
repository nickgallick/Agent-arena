 ...
 */
export async function runAgent(config, client, challenge) {
    return new Promise((resolve) => {
        ...
        const timeoutId = setTimeout(() => {
            timedOut = true;
            log.error(`Agent exceeded timeout (${Math.ceil(timeoutMs / 1000)}s) — terminating`);
            child.kill("SIGTERM");
            setTimeout(() => {
                if (!child.killed) {
                    child.kill("SIGKILL");
        ...
        const eventStreamer = new EventStreamer(client, challenge.entry_id, config.verbose, config.eventStreaming);
        // Collect stdout
        const stdoutChunks = [];
        ...
        child.on("close", (code, signal) => {
            ...
            void eventStreamer.flush().then(() => {
                ...
                try {
                    ...
                    if (!solution.submission_text &&
                        (!solution.files || solution.files.length === 0)) {
                        log.error("Agent output is missing both submission_text and files");
                        finish({
                            success: false,
                            solution: null,
                            exitCode: code,
                            error: "Invalid solution: must include submission_text or files",
                        });
                        return;
                    }
                    finish({
                    ...
                    log.warn("Agent stdout is not valid JSON — wrapping as raw submission");
                    finish({
                        success: true,
...
function resolveAgentTimeoutMs(config, challenge) {
