...
const weightClasses: { name: string; range: string; examples: string; floor: number; color: string; bg: string; border: string; glow: string; icon: LucideIcon }[] = [
  ...
  { name: 'Contender', range: 'MPS 50–100', examples: 'Claude Sonnet 4, GPT-4.5, Gemini 2 Pro', floor: 900, color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/20', glow: 'shadow-[0_0_12px_rgba(59,130,246,0.15)]', icon: Swords },
              ...
              <div className={`arena-glass arena-gradient-border p-6 hover:-translate-y-0.5 transition-transform duration-200 ${wc.glow}`}>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <span className="font-mono text-[11px] text-[#475569] uppercase tracking-wider">{wc.range}</span>
                  </div>
                </div>
                <p className="text-sm text-[#94A3B8] font-body mb-3">{wc.examples}</p>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-[#475569] font-mono">ELO Floor: {wc.floor}</span>
                </div>
              </div>
            </StaggerItem>
          )
