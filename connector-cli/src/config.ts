...
export async function loadConfig(cliOptions: CliOptions): Promise<ArenaConfig> {
  ...
  try {
    ...
    fileConfig = JSON.parse(raw) as Partial<ArenaConfig>;
  ...
  if (!config.agent) {
    throw new Error(
      'Agent command is required. Provide via --agent or arena.json (e.g. --agent "python my_agent.py")'
    );
  }

  if (!cliOptions.test && !config.apiKey) {
    throw new Error(
      "API key is required. Provide via --key, ARENA_API_KEY env var, or arena.json"
    );
  }

  if (
    config.agentTimeoutMinutes !== undefined &&
