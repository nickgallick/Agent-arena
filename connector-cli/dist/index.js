...
program
    ...
    .option("--agent-timeout-minutes <minutes>", "Override agent timeout in minutes (defaults to the challenge time limit)", (value) => Number(value))
...
async function run(options) {
    ...
    log.dim(`  Agent timeout: ${config.agentTimeoutMinutes ?? "challenge time limit"}` +
        `${config.agentTimeoutMinutes ? " min" : ""}`);
    // Initial ping to verify connectivity + auth
    log.info("Connecting to Arena...");
    const pingResult = await client.ping();
    if (!pingResult) {
        throw new Error("Failed to connect to Arena. Check your API key and network.");
    }
...
async function runTestMode(config) {
    const fakeChallenge = {
        ...
        prompt: "Return a tiny valid submission that proves your agent can read stdin, optionally emit stderr events, and write a result to stdout.",
    ...
    log.dim(`  Event streaming: ${config.eventStreaming ? "parsed locally only" : "off"}`);
...
// Challenge Polling
// ----------------------------------------------------------
async function pollLoop(client, config, isShuttingDown, setActiveChallenge) {
        ...
        try {
            ...
            if (result?.challenges && result.challenges.length > 0) {
                for (const challenge of result.challenges) {
                    if (isShuttingDown())
                        break;
                    await handleChallenge(client, config, challenge, setActiveChallenge);
                }
            }
        }
        catch (err) {
...
// Challenge Handler
// ----------------------------------------------------------
async function handleChallenge(client, config, challenge, setActiveChallenge) {
    log.challenge(`Challenge assigned: "${challenge.title}" [${challenge.category}]`);
    log.info(`Time limit: ${challenge.time_limit_minutes} min | Entry: ${challenge.entry_id}`);
    ...
    try {
        ...
        if (result.success && result.solution) {
            ...
            log.error(`Agent failed: ${result.error ?? `exit code ${result.exitCode}`}`);
        }
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : "unknown error";
