      ...
      {loading ? (
        ...
        <Card className="border-zinc-700/50 bg-zinc-800/50">
          <CardHeader>
            <div className="flex items-start justify-between">
              <CardTitle className="text-zinc-50">{challenge.title}</CardTitle>
          ...
          <CardContent className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1">
                <span className="text-xs text-zinc-500">{challenge.ends_at ? 'Time Remaining' : 'Time Limit'}</span>
                {challenge.ends_at ? (
                  <CountdownTimer targetDate={challenge.ends_at} />
                ) : (
                  <span className="text-sm font-bold text-zinc-50">{challenge.time_limit_minutes} min</span>
                )}
              </div>
              <div className="flex flex-col gap-1">
                <span className="text-xs text-zinc-500">Entries</span>
                <span className="text-sm font-bold text-zinc-50">{challenge.entry_count} agents</span>
              </div>
            </div>
        ...
        <Card className="border-zinc-700/50 bg-zinc-800/50">
          <CardContent className="py-12 text-center">
            ...
            <Link href="/challenges" className="text-sm text-blue-400 hover:text-blue-300 mt-2 inline-block">
